...
> Diagnose omapad at runtime - a pad that does nothing, a button doing the
> wrong thing, a surface that will not draw, a pointer that drifts, a pad that
> will not hand over to a game, or a binding that silently does nothing. Use
> when something works in the tests but not on the machine. Walks the
> diagnostic ladder in order instead of guessing.
...
- Daemon says open, nothing on screen → it is the panel. See the three silent
  QML failures in [`pad-surface.md`](pad-surface.md); the shell log is the
...
| Pointer drifts a little | not enough dead zone on that stick - raise `[pointer] left_deadzone` (or `right_deadzone`) 0.10 → 0.15, or move the bar on Controller > Sticks with the pointer live under the menu |
...
| The tick sticks on, and stays on until something buzzes again | the pad is told to run its motors until told otherwise, and the stop the kernel owes the finished effect went missing - reported under a game and a browser, which are where a second force-feedback client is playing into the same device. The tick now writes its own stop rather than waiting for that one. If it comes back, it is somebody else's effect - `omapad ctl status` says `pad=app`, and stopping the daemon does not stop the motor |
| The pointer never goes away on a press | `[pointer] hide_on_press` asks Hyprland for the hide it already does for a keystroke, so with `cursor:hide_on_key_press` off nothing here can work. The daemon says so once at startup - `journalctl --user -u omapad \| grep pointer`. It cannot be turned on with `hyprctl keyword` on this build; see [`lua.md`](../conventions/lua.md) |
| The pointer is invisible everywhere, and stopping the daemon does not bring it back | not omapad: `omarchy-screensaver` sets `cursor:invisible` while it is up and puts it back when it is dismissed. `pgrep -f omarchy-screensaver` and `hyprctl getoption cursor:invisible` answer it in two lines |
...
| A badge prints typed text | the label has no art - see [`pad-badge-art.md`](pad-badge-art.md) |
