...
| `shapes/dial-cap.svg`, `shapes/dial-pointer.svg`, `shapes/dial-notch.svg`, `shapes/dial-end.svg`, `shapes/dial-detent.svg`, `-lit` | The knob, a Braun T 1000 control drawn in the clock's weights and no others - the cap's edge is the clock's double case, a mark of the scale is the sweep hand's width, an end is an hour baton in outline with its foot left open, the scale's arc running on under it to close it, and the index is that baton solid; `DialsShareTheClocksWeights` holds it there. The cap never turns; the others do - the index at the value, one mark of the printed scale and the longer mark at each end of it, and on a stepped ring a detent per place instead of the marks, with a detent or an end filled (`-lit`) once the value has stood on it. The turning ones are drawn standing at twelve on the dial's own canvas, because a silhouette that is the same at every value is a shape and the value only decides the angle. `dial-face.svg` is the gauge's alone now. |
...
| `shapes/travel-*.svg` | **A slider's scale is the knob's, unrolled** ([92](../decisions/92-a-tuning-scale.md)): `notch` a graduation, `end` an end baton with its foot open for the line to close, `detent` a place a stepped value can stand, `end-lit` and `detent-lit` the same two filled once the value has stood on them, `needle` the value, and `ghost` the needle's tail alone, hung under the line where a press found it - three weights wide under an end or a detent, one (`ghost-thin`) under a graduation - each the ring's figure rounded to whole line weights, because a straight figure lands on pixels where a turned one never can. **A card of rows keeps its own cap**: `end-open`, one at each end - eleven weights across, with the line's own weight of air between its arms for the line to run through. Its row in force is the line itself drawn three weights wide, which is geometry, not a drawing. Every side of every figure is a whole number of ten-unit line weights, so a figure lands on whole pixels at every scale. The line itself is still geometry: it is as long as the tile, and a drawing cannot stretch. |
...
- **Draw on a canvas `Metrics.badgeGrid` divides.** A surface reserves a badge
  ...
  **The strokes on a line are exempt, and the exemption is this rule read
  ...
  own weight, or three of it, and takes the height the drawing asks for, so
  what has to be whole is every side - a whole number of ten-unit line
  weights - and then the box is whole pixels on both sides at every scale
  there is. The same test checks that instead.
