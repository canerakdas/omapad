...
| `shapes/dial-cap.svg`, `shapes/dial-pointer.svg`, `shapes/dial-notch.svg`, `shapes/dial-end.svg` | The knob, a Braun T 1000 control drawn in the clock's weights and no others - the cap's edge is the clock's double case, a mark of the scale is the sweep hand's width, an end is an hour baton in outline with its foot left open, the scale's arc running on under it to close it, and the index is that baton solid; `DialsShareTheClocksWeights` holds it there. The cap never turns; the other three do - the index at the value, one mark of the printed scale and the longer mark at each end of it. The turning ones are drawn standing at twelve on the dial's own canvas, because a silhouette that is the same at every value is a shape and the value only decides the angle. `dial-face.svg` is the gauge's alone now. |
