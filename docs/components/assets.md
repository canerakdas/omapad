...
| `shapes/clock-*.svg` | Everything on a clock face and on the chronograph that shares it: the rim, its twelve marks, the hub, the hour and minute hands, the sweep hand, and a register's disc and hand. Drawn on the dial's own 40-unit canvas so the two circles a page may hold are one circle, with a hairline case and a chapter ring inside it against the dial's one band three units thick - this one has five hands and twelve batons inside it rather than a dot - except the register's pair, which is drawn on the register's own 40 so a hand there is measured against the circle it turns in. A hand stands at twelve, pinned at 20,20 where the hub is; the panel turns the whole face-sized box about its middle and names no length, weight or corner. What is left to it is where the three registers sit and how big they are. |
